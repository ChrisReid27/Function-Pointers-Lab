...
int my_comparer(const void *this, const void *that)
{
  ...
  //Primary Sort: Integer Priority in Descending Order
  if (a->priority != b->priority) {
        return b->priority - a->priority;
  }

  //Secondary Sort: Integer Arrival Time in Ascending Order
...
int main(int argc, char *argv[])
{
  ...
  printf("The Sorted Processes (Priority Descending, Arrival Time Ascending):\n");
	for (int i = 0; i < P_SIZE; i++) {
		    printf("%d (%d, %d)\n",
				processes[i].pid,
				processes[i].priority, processes[i].arrival_time);
